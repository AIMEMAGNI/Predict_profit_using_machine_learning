...
st.info("For proper visualisations of this app, please change the theme to dark, by clicking the menu icon in upright corner> settings > Theme.")
...
if selected == "Project":
    ...
    st.error('This dataset only shows the expenditures and profit from an unkown company. It does not show their investiments or capital. However it was used to showcase how Machine Learning model would handle the task because it uses the similar approach to solve the problem.')
...
if selected == "Demo":
    ...
    st.warning('This demo uses the model only trained on dataset showed in Project section, so the predictions might not be too practical.')
